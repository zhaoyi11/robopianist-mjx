...
from robopianist_mjx.mjx_env import MjxState
import numpy as np
from dm_control import mjcf

from robopianist_mjx.music import midi_file, midi_message
...
class MidiModule:
    ...
    def append(
        ...
        if sustain_change & ~self._prev_sustain_activation:
            timestep_events.append(midi_message.SustainOn(time=mjx_state.time))
            if self._sustain_on_callback is not None:
                self._sustain_on_callback()
        if sustain_change & ~sustain_activation:
            timestep_events.append(midi_message.SustainOff(time=mjx_state.time))
